...
function HasilPeserta() {
	...
	const pct = sesi.maxSkor
		? Math.round((sesi.skorTotal! / sesi.maxSkor) * 100)
		: 0;
					...
					{ujian.showResult ? (
						<>
							...
							<div className="text-sm text-muted-foreground">
								dari {sesi.maxSkor} ({pct}%)
							</div>
						</>
					) : (
						<div className="text-muted-foreground">
							Hasil tidak ditampilkan oleh penyelenggara.
						</div>
					)}
				</CardContent>
			</Card>
						...
						{sesi.jawaban.map((j, i) => {
							...
							if (!soal) {
								return (
									...
									>
										Soal #{i + 1} tidak tersedia lagi di modul.
									</div>
								);
							}
							const benarIds = soal.jawaban
								.filter((x) => x.benar)
								.map((x) => x.id);
								...
								<div key={i} className="rounded border p-3 space-y-2 text-sm">
										...
										<ul className="ml-4 list-disc text-xs">
											{soal.jawaban.map((opt) => (
												...
												>
													...
													{opt.benar && " ✓"}
													{j.jawabanIds.includes(opt.id) && " (jawaban Anda)"}
												</li>
											))}
										</ul>
									)}
									{soal.pembahasan && (
										<div className="rounded bg-muted p-2 text-xs">
											...
											<RichView html={soal.pembahasan} className="inline" />
										</div>
									)}
								</div>
							);
